...
export default function Settings() {
  ...
  // Disparition automatique des erreurs après 3 secondes
  useEffect(() => {
  ...
  const handleSaveProfile = async (e) => {
    ...
    if (!formData.shopName.trim()) {
      setErrors({ shopName: "Le nom est obligatoire" });
      return;
    }
  ...
  const handleChangePassword = async (e) => {
    ...
    // Validation uniquement sur le NOUVEAU mot de passe (6 car. min)
    if (passData.newPassword.length < 6) {
      pErrors.newPassword = "Le nouveau mot de passe doit faire au moins 6 caractères.";
    }
      ...
      <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/95 backdrop-blur-xl">
        <div className="glass-card w-full max-w-md p-8 rounded-[3rem] border-white/10 shadow-2xl relative">
          ...
          <div className="flex flex-col items-center text-center">
            ...
            <h3 className="text-2xl font-black italic uppercase mb-2 tracking-tighter">Accès Restreint</h3>
  ...
  return (
    <div className="p-4 max-w-6xl mx-auto min-h-screen text-white pb-10 font-sans">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
        ...
        {/* FORMULAIRE 1 : INFORMATIONS BOUTIQUE */}
        <form onSubmit={handleSaveProfile} className="glass-card p-8 rounded-[2.5rem] space-y-6 border-white/5 shadow-2xl">
          ...
          <div className="space-y-4">
            <div>
              ...
              {errors.shopName && <p className="text-[9px] text-red-500 font-bold uppercase mt-1 flex items-center gap-1 italic animate-in fade-in"><AlertTriangle size={10}/> {errors.shopName}</p>}
            </div>
            
            <IceInput label="Téléphone" value={formData.phone} onChange={(e) => setFormData({...formData, phone: e.target.value})} />
        ...
        {/* FORMULAIRE 2 : SECURITE MOT DE PASSE */}
        <form onSubmit={handleChangePassword} className="glass-card p-8 rounded-[2.5rem] space-y-6 border-white/5 shadow-2xl bg-gradient-to-br from-white/[0.02] to-transparent">
          <div className="flex items-center gap-3 mb-2">
            <div className="p-2 bg-red-500/10 rounded-lg"><KeyRound size={18} className="text-red-500" /></div>
            <h2 className="text-sm font-black uppercase italic tracking-widest">Changer le mot de passe</h2>
          </div>

          <div className="space-y-5">
            {/* ANCIEN MOT DE PASSE (SANS AUTO-FILL) */}
            <div>
              <IceInput 
                ...
                autoComplete="one-time-code" // Astuce contre l'auto-remplissage
                placeholder="Entrez votre mot de passe actuel"
                required
              />
              {errors.auth && <p className="text-[9px] text-red-500 font-bold uppercase mt-1 flex items-center gap-1 italic animate-in fade-in"><AlertTriangle size={10}/> {errors.auth}</p>}
            </div>
            ...
            <div>
              <IceInput 
                ...
                required
              />
              {errors.newPassword && <p className="text-[9px] text-red-500 font-bold uppercase mt-1 flex items-center gap-1 italic animate-in fade-in duration-300"><AlertTriangle size={10}/> {errors.newPassword}</p>}
            </div>
          </div>

          <button className="w-full border border-white/10 hover:bg-white/5 text-white font-black py-4 rounded-2xl uppercase text-xs transition-all italic tracking-widest mt-4">
