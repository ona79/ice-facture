...
export default function History() {
  ...
  // --- LOGIQUE DE FILTRE CORRIGÉE ET SÉCURISÉE ---
  const filteredInvoices = invoices.filter(inv => {
    const term = searchTerm.toLowerCase().trim();
    
    // 1. On récupère le numéro affiché
    const displayNum = formatInvoiceDisplay(inv).toLowerCase();
    
    // 2. On récupère le nom du client (SECURITÉ : Valeur par défaut si null/undefined)
    const clientName = (inv.customerName || "client passager").toLowerCase();
    
    // 3. On récupère le numéro de facture original (celui de la base de données)
    const originalNum = (inv.invoiceNumber || "").toLowerCase();

    // Vérification si le terme de recherche match avec le NOM ou le NUMÉRO
    const isSearchMatch = displayNum.includes(term) || clientName.includes(term) || originalNum.includes(term);
    ...
    <div className="p-4 md:p-6 max-w-5xl mx-auto min-h-screen text-white font-sans">
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md">
          <div className="glass-card w-full max-w-md p-8 rounded-[2.5rem] border-white/10 relative">
            ...
            <div className="flex flex-col items-center text-center">
              ...
              <p className="text-white/30 text-[10px] uppercase mb-8 tracking-widest">Entrez le code pour supprimer la facture {modalDelete.num}</p>
              <form onSubmit={handleVerifyAndAction} className="w-full space-y-4 text-left">
                <IceInput label="Password" type="password" value={password} onChange={(e) => setPassword(e.target.value)} required />
                <button type="submit" className="w-full py-4 rounded-2xl font-black bg-red-500 text-white uppercase shadow-lg shadow-red-500/20">Supprimer</button>
              </form>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/95">
          <div className="glass-card w-full max-w-md p-8 rounded-[2.5rem] border-orange-500/20 relative">
            ...
            <div className="text-center">
              ...
              <h3 className="text-2xl font-black italic uppercase">Dette : {modalPay.invoice.customerName || 'Client'}</h3>
              ...
              <form onSubmit={handleSettleDebt} className="space-y-4 text-left">
                ...
                <button type="submit" className="w-full py-4 rounded-2xl font-black bg-orange-500 text-white uppercase tracking-widest">Confirmer</button>
              </form>
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
          ...
          <p className="text-ice-400 text-[9px] font-black uppercase tracking-[0.3em] mt-1 opacity-50">Recherche par Nom ou N° Facture</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-10">
        ...
        <div className="glass-card p-6 rounded-[2.5rem] border-white/5 bg-white/[0.01] flex items-center justify-between">
          <div><p className="text-[10px] font-black uppercase text-white/20 mb-2">Ventes Listées</p><h2 className="text-4xl font-black italic text-white">{filteredInvoices.length}</h2></div>
              ...
              <div key={inv._id} className={`p-4 rounded-[1.5rem] border flex flex-wrap justify-between items-center gap-4 transition-all ${reste > 0 ? 'border-orange-500/30 bg-orange-500/[0.03]' : 'border-white/5 bg-white/[0.02]'}`}>
                ...
                <div className="flex items-center gap-6 ml-auto">
                  <div className="text-right">
                    ...
                    <p className="text-[8px] text-white/20 font-black uppercase">{new Date(inv.createdAt).toLocaleDateString('fr-FR')} à {new Date(inv.createdAt).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}</p>
                  </div>
